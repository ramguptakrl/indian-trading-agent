...
def record_actual_trade(
    *,
    ...
    if active_mode == "SWING" and side != "LONG":
        raise ValueError("SWING is LONG-only cash/delivery in the active architecture")
...
def get_actual_trade(trade_id: str, *, db_path: str | None = None) -> dict[str, Any]:
    ...
    result["exits"] = [dict(x) for x in exits]
    return result
...
def close_actual_trade(
    *,
    ...
    gross = _gross_pnl(trade["direction"], float(trade["avg_entry_price"]), float(exit_price), close_qty)
    estimated_charges = float(economics["charges"]["total"])
    ...
    with _connect(db_path) as conn:
        conn.execute(
            ...
            INSERT INTO tb_actual_trade_exits(
                ...
                broker_order_ref, notes, created_at
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                exit_id,
...
def mark_actual_trade(
    *,
    ...
    estimated_open_net = float(economics["net_pnl"])
    ...
    return {
        ...
        "estimated_open_charges_if_closed_now": float(economics["charges"]["total"]),
